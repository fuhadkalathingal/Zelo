                ...
                <div className="grid grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4 mb-6">
                    {CATEGORIES.map((cat) => (
                        <button key={cat.name} onClick={() => setActiveCategory(cat.name)} className="flex flex-col items-center gap-2">
                            <div className={`w-full aspect-square rounded-2xl ${cat.bg} border ${activeCategory === cat.name ? 'border-emerald-400' : 'border-white'} shadow-sm flex items-center justify-center text-4xl`}>
                                {cat.icon}
                            </div>
